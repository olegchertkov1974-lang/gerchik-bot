...
class TelegramNotifier {
  constructor() {
    ...
    this._startTime = Math.floor(Date.now() / 1000); // Track bot start time for stale callback detection
  ...
  async _handleUpdate(update) {
    ...
    if (update.callback_query) {
      ...
      // Пропускаем callback'и от сообщений созданных ДО старта бота (накопились пока бот был выключен)
      const msgDate = cb.message?.date || 0;
      if (msgDate > 0 && msgDate < this._startTime) {
        logger.warn(`Telegram: пропускаем устаревший callback (сообщение от ${new Date(msgDate*1000).toISOString()}): ${cb.data}`);
        return;
      }
  ...
  async _handleCallback(cb) {
    ...
    // ── Настройки ──
      ...
      await this._cmdSetRisk();
    } else if (data.startsWith('risk_')) {
      await this._applyRisk(data.replace('risk_', ''));
    } else if (data === 'set_leverage') {
      await this._cmdSetLeverage();
    } else if (data.startsWith('lev_')) {
      await this._applyLeverage(data.replace('lev_', ''));
    } else if (data === 'set_maxpos') {
      await this._cmdSetMaxPos();
    } else if (data.startsWith('maxpos_')) {
      await this._applyMaxPos(data.replace('maxpos_', ''));
    } else if (data === 'set_ai') {
      await this._cmdSetAI();
    } else if (data === 'ai_on') {
      await this._applyAIToggle(true);
    } else if (data === 'ai_off') {
      await this._applyAIToggle(false);
    } else if (data === 'set_ai_conf') {
      await this._cmdSetAIConf();
    } else if (data.startsWith('aiconf_')) {
      await this._applyAIConf(data.replace('aiconf_', ''));
    } else if (data === 'set_daily_loss') {
      await this._cmdSetDailyLoss();
    } else if (data.startsWith('dloss_')) {
      await this._applyDailyLoss(data.replace('dloss_', ''));
    } else if (data === 'manage_pairs') {
      await this._cmdManagePairs();
    } else if (data.startsWith('pair_toggle_')) {
      await this._applyPairToggle(data.replace('pair_toggle_', ''));
    } else if (data.startsWith('mgr_toggle_')) {
      await this._applyMgrToggle(data.replace('mgr_toggle_', ''));
    } else if (data === 'noop') {
      // locked pair — ignore
    } else if (data === 'check_sltp') {
      await this._cmdCheckSlTp();
    } else if (data.startsWith('fix_sltp_')) {
      await this._applyFixSlTp(data.replace('fix_sltp_', ''));
    }
  }
  ...
  async _cmdPositions() {
    ...
    buttons.push([{ text: '🛡 Проверить SL/TP', callback_data: 'check_sltp' }]);
    buttons.push([
      { text: '🔴 Закрыть все', callback_data: 'close_all' },
      { text: '◀️ Меню', callback_data: 'menu' },
    ]);
  ...
  async _cmdWeekly() {
    ...
    const fs = require('fs');
    try {
      await this.sendMessage('📊 <b>Генерация недельного отчёта...</b>');
      const today = new Date().toISOString().slice(0, 10);
      const botDir = path.resolve(__dirname, '..');
      execSync(`node scripts/generate-weekly-report.js ${today}`, { cwd: botDir, timeout: 30000 });
      const reportPath = path.join(botDir, 'reports', `week-${today}.md`);
      const content = fs.readFileSync(reportPath, 'utf8');
      // Send in chunks of 4000 chars
      for (let i = 0; i < content.length; i += 4000) {
        const chunk = content.slice(i, i + 4000);
        await this.sendMessage('<pre>' + chunk.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') + '</pre>');
      }
    } catch (err) {
      await this.sendMessage(`❌ Ошибка недельного отчёта: ${err.message}`);
    }
  }
  ...
  async _cmdSettings() {
    ...
    const leverage = process.env.LEVERAGE || '20';
    const aiEnabled = process.env.AI_FILTER_ENABLED !== 'false';
    const aiConf = process.env.AI_MIN_CONFIDENCE || '60';
    const dailyLoss = process.env.DAILY_LOSS_LIMIT_PCT || '20';
    ...
    const msg =
      ...
      `🤖 AI фильтр: <code>${aiEnabled ? 'ВКЛ ✅' : 'ВЫКЛ ❌'}</code>\n` +
      `🎯 Мин AI уверенность: <code>${aiConf}%</code>\n` +
      `🛡 Дневной лимит потерь: <code>${dailyLoss}%</code>\n\n` +
      `<i>Нажми кнопку чтобы изменить</i>`;

    const keyboard = {
      inline_keyboard: [
        [
          { text: `💰 Риск (${rm.riskPct}%)`, callback_data: 'set_risk' },
          { text: `📊 Плечо (${leverage}x)`, callback_data: 'set_leverage' },
        ],
        [
          { text: `🔢 Макс позиций (${rm.maxConcurrent})`, callback_data: 'set_maxpos' },
          { text: `🛡 Лимит потерь (${dailyLoss}%)`, callback_data: 'set_daily_loss' },
        ],
        [
          { text: `🤖 AI: ${aiEnabled ? 'ВКЛ ✅' : 'ВЫКЛ ❌'}`, callback_data: 'set_ai' },
          { text: `🎯 AI уверен. (${aiConf}%)`, callback_data: 'set_ai_conf' },
        ],
        [
          { text: '🔧 Управление парами', callback_data: 'manage_pairs' },
        ],
        [
          { text: '◀️ Меню', callback_data: 'menu' },
        ],
  ...
  // ── Риск ──
  async _cmdSetRisk() {
    const cur = process.env.RISK_PCT || '1';
    const keyboard = {
      inline_keyboard: [
        [
          { text: '0.5%', callback_data: 'risk_0.5' },
          { text: '1%', callback_data: 'risk_1' },
          { text: '2%', callback_data: 'risk_2' },
        ],
        [
          { text: '3%', callback_data: 'risk_3' },
          { text: '4%', callback_data: 'risk_4' },
          { text: '5%', callback_data: 'risk_5' },
        ],
        [{ text: '◀️ Назад', callback_data: 'settings' }],
      ],
    };
    await this._sendWithKeyboard(`💰 <b>Риск на сделку</b>\nТекущий: <code>${cur}%</code>\n\nВыбери новое значение:`, keyboard);
  }

  async _applyRisk(val) {
    const n = parseFloat(val);
    if (isNaN(n) || n < 0.5 || n > 5) { await this.sendMessage('❌ Недопустимое значение'); return; }
    this._updateEnv('RISK_PCT', String(n));
    if (this._bot && this._bot.riskManager) this._bot.riskManager.riskPct = n;
    await this.sendMessage(`✅ <b>Риск установлен: ${n}%</b>\nСохранено в .env`);
    await this._cmdSettings();
  }

  // ── Плечо ──
  async _cmdSetLeverage() {
    const cur = process.env.LEVERAGE || '20';
    const keyboard = {
      inline_keyboard: [
        [
          { text: '5x', callback_data: 'lev_5' },
          { text: '10x', callback_data: 'lev_10' },
          { text: '15x', callback_data: 'lev_15' },
        ],
        [
          { text: '20x', callback_data: 'lev_20' },
          { text: '25x', callback_data: 'lev_25' },
          { text: '30x', callback_data: 'lev_30' },
        ],
        [{ text: '◀️ Назад', callback_data: 'settings' }],
      ],
    };
    await this._sendWithKeyboard(`📊 <b>Плечо</b>\nТекущее: <code>${cur}x</code>\n\nВыбери новое значение:\n<i>⚠️ Смена плеча перезапустит бота</i>`, keyboard);
  }

  async _applyLeverage(val) {
    const n = parseInt(val);
    if (isNaN(n) || n < 1 || n > 50) { await this.sendMessage('❌ Недопустимое значение'); return; }
    this._updateEnv('LEVERAGE', String(n));
    await this.sendMessage(`✅ <b>Плечо установлено: ${n}x</b>\nСохранено в .env\n<i>Перезапускаю бота...</i>`);
    const { execSync } = require('child_process');
    try { execSync('pm2 restart gerchik-bot --update-env', { timeout: 10000 }); } catch (e) { /* ok */ }
  }

  // ── Макс позиций ──
  async _cmdSetMaxPos() {
    const cur = process.env.MAX_POSITIONS || '10';
    const keyboard = {
      inline_keyboard: [
        [
        ...
        [
          ...
          { text: '8', callback_data: 'maxpos_8' },
          { text: '10', callback_data: 'maxpos_10' },
        ],
        [{ text: '◀️ Назад', callback_data: 'settings' }],
      ],
    };
    await this._sendWithKeyboard(`🔢 <b>Макс позиций одновременно</b>\nТекущее: <code>${cur}</code>\n\nВыбери новое значение:`, keyboard);
  }

  async _applyMaxPos(val) {
    const n = parseInt(val);
    if (isNaN(n) || n < 1 || n > 10) { await this.sendMessage('❌ Недопустимое значение'); return; }
    this._updateEnv('MAX_POSITIONS', String(n));
    if (this._bot && this._bot.riskManager) this._bot.riskManager.maxConcurrent = n;
    await this.sendMessage(`✅ <b>Макс позиций: ${n}</b>\nСохранено в .env`);
    await this._cmdSettings();
  }

  // ── AI вкл/выкл ──
  async _cmdSetAI() {
    const cur = process.env.AI_FILTER_ENABLED !== 'false';
    const keyboard = {
      inline_keyboard: [
        [
          { text: '✅ Включить AI', callback_data: 'ai_on' },
          { text: '❌ Выключить AI', callback_data: 'ai_off' },
        ],
        [{ text: '◀️ Назад', callback_data: 'settings' }],
      ],
    };
    await this._sendWithKeyboard(`🤖 <b>AI фильтр</b>\nСейчас: <code>${cur ? 'ВКЛ' : 'ВЫКЛ'}</code>\n\n<i>AI анализирует каждый сигнал перед входом</i>`, keyboard);
  }

  async _applyAIToggle(enabled) {
    this._updateEnv('AI_FILTER_ENABLED', enabled ? 'true' : 'false');
    if (this._bot && this._bot.aiFilter) this._bot.aiFilter.enabled = enabled && !!this._bot.aiFilter.apiKey;
    await this.sendMessage(`✅ <b>AI фильтр ${enabled ? 'включён' : 'выключен'}</b>\nСохранено в .env`);
    await this._cmdSettings();
  }

  // ── AI уверенность ──
  async _cmdSetAIConf() {
    const cur = process.env.AI_MIN_CONFIDENCE || '60';
    const keyboard = {
      inline_keyboard: [
        [
          { text: '50%', callback_data: 'aiconf_50' },
          { text: '60%', callback_data: 'aiconf_60' },
          { text: '65%', callback_data: 'aiconf_65' },
        ],
        [
          { text: '70%', callback_data: 'aiconf_70' },
          { text: '75%', callback_data: 'aiconf_75' },
          { text: '80%', callback_data: 'aiconf_80' },
        ],
        [{ text: '◀️ Назад', callback_data: 'settings' }],
      ],
    };
    await this._sendWithKeyboard(`🎯 <b>Мин. уверенность AI</b>\nТекущая: <code>${cur}%</code>\n\nСигналы ниже порога — отклоняются:`, keyboard);
  }

  async _applyAIConf(val) {
    const n = parseInt(val);
    if (isNaN(n) || n < 0 || n > 100) { await this.sendMessage('❌ Недопустимое значение'); return; }
    this._updateEnv('AI_MIN_CONFIDENCE', String(n));
    await this.sendMessage(`✅ <b>Мин. AI уверенность: ${n}%</b>\nСохранено в .env`);
    await this._cmdSettings();
  }

  // ── Дневной лимит потерь ──
  async _cmdSetDailyLoss() {
    const cur = process.env.DAILY_LOSS_LIMIT_PCT || '20';
    const keyboard = {
      inline_keyboard: [
        [
          { text: '5%', callback_data: 'dloss_5' },
          { text: '10%', callback_data: 'dloss_10' },
          { text: '15%', callback_data: 'dloss_15' },
        ],
        [
          { text: '20%', callback_data: 'dloss_20' },
          { text: '25%', callback_data: 'dloss_25' },
          { text: '30%', callback_data: 'dloss_30' },
        ],
        [{ text: '◀️ Назад', callback_data: 'settings' }],
      ],
    };
    await this._sendWithKeyboard(`🛡 <b>Дневной лимит потерь</b>\nТекущий: <code>${cur}%</code>\n\nПри достижении бот остановит торговлю:`, keyboard);
  }

  async _applyDailyLoss(val) {
    const n = parseInt(val);
    if (isNaN(n) || n < 1 || n > 50) { await this.sendMessage('❌ Недопустимое значение'); return; }
    this._updateEnv('DAILY_LOSS_LIMIT_PCT', String(n));
    await this.sendMessage(`✅ <b>Лимит потерь: ${n}%</b>\nСохранено в .env`);
    await this._cmdSettings();
  }

  // ── Управление парами ──
  async _cmdManagePairs() {
    if (!this._bot) return;
    const ALL_SCAN_PAIRS = [
    ...
    const PAIRS = this._bot.getPairs ? this._bot.getPairs() : [];
    const positions = this._bot.positions;

    let msg = '🔧 <b>Управление парами</b>\n';
    msg += `<i>Активных: ${PAIRS.length}/50</i>\n\n`;
    const buttons = [];

    for (const pair of ALL_SCAN_PAIRS) {
      const inActive = PAIRS.includes(pair);
      const hasPos = positions.has(pair);
      const icon = hasPos ? '🔒' : (inActive ? '✅' : '·');
      msg += `${icon} ${pair}\n`;
      if (hasPos) {
        buttons.push([{ text: `🔒 ${pair} — позиция`, callback_data: 'noop' }]);
      } else {
        buttons.push([{
          text: `${icon} ${pair} — ${inActive ? 'выключить' : 'включить'}`,
          callback_data: `mgr_toggle_${pair.replace('/', '_')}`,
        }]);
      }
    }

    buttons.push([{ text: '◀️ Назад', callback_data: 'settings' }]);
    await this._sendWithKeyboard(msg, { inline_keyboard: buttons });
  }

  async _applyMgrToggle(pairKey) {
    if (!this._bot) return;
    const pair = pairKey.replace('_', '/');
    const PAIRS = this._bot.getPairs ? this._bot.getPairs() : [];
    if (PAIRS.includes(pair)) {
      const result = this._bot.removeActivePair(pair);
      if (result.error) {
        await this.sendMessage(`⚠️ ${result.error}`);
      } else {
        await this.sendMessage(`❌ <b>${pair}</b> выключена`);
      }
    } else {
      const result = this._bot.addActivePair(pair);
      if (result.error) {
        await this.sendMessage(`⚠️ ${result.error}`);
      } else {
        await this.sendMessage(`✅ <b>${pair}</b> включена`);
      }
    }
    await this._cmdManagePairs();
  }

    // ── Утилита: сохранение в .env + process.env ──
  _updateEnv(key, value) {
    try {
      ...
      const logger = require('./logger');
      logger.error(`_updateEnv(${key}): ${err.message}`);
    }
  }
  ...
  async sendDailyReport(report) {
    ...
    const msg =
      ...
      `└ 💰 Нет маржи: ${r.orders.marginFailures || 0}\n\n` +
  ...
  async _cmdCheckSlTp() {
    if (!this._bot) return;
    await this.sendMessage('🔍 <b>Проверяю позиции...</b>');

    let positions;
    try {
      positions = await this._bot.exchange.fetchOpenPositions();
    } catch (e) {
      await this.sendMessage('❌ Ошибка получения позиций: ' + e.message);
      return;
    }

    const open = positions.filter(p => Math.abs(parseFloat(p.contracts || p.size || 0)) > 0);
    if (!open.length) {
      await this.sendMessage('📋 <b>Нет открытых позиций</b>');
      return;
    }

    let msg = '🛡 <b>Статус SL/TP по позициям</b>\n\n';
    const buttons = [];
    let hasMissing = false;

    for (const p of open) {
      const pair  = p.symbol.replace(':USDT', '').replace(':USD', '');
      const sl    = parseFloat(p.stopLossPrice || 0);
      const tp    = parseFloat(p.takeProfitPrice || 0);
      ...
      msg += `${side} <b>${pair}</b>\n`;
      msg += `Вход: <code>${entry}</code> | PnL: <code>${pnlSign}${pnl.toFixed(2)}</code>\n`;
      msg += `SL: ${slIcon} ${sl > 0 ? '<code>' + sl + '</code>' : 'НЕТ'}  `;
      msg += `TP: ${tpIcon} ${tp > 0 ? '<code>' + tp + '</code>' : 'НЕТ'}\n\n`;

      if (sl === 0 || tp === 0) {
        hasMissing = true;
        const pairKey = pair.replace('/', '_');
        buttons.push([{
          text: `🔧 Установить SL/TP для ${pair}`,
          callback_data: `fix_sltp_${pairKey}`,
        }]);
      }
    }

    if (!hasMissing) msg += '✅ <b>Все позиции защищены</b>';
    buttons.push([{ text: '◀️ Меню', callback_data: 'menu' }]);

    await this._sendWithKeyboard(msg, { inline_keyboard: buttons });
  }

  async _applyFixSlTp(pairKey) {
    if (!this._bot) return;
    const pair = pairKey.replace(/_/g, '/').replace('/USDT', '/USDT');
    // Handle ONDO_USDT -> ONDO/USDT
    const pairNorm = pairKey.includes('_') ? pairKey.replace('_', '/') : pairKey;
    const pairFinal = pairNorm.includes('USDT') ? pairNorm : pairNorm + '/USDT';

    await this.sendMessage(`⏳ Рассчитываю SL/TP для <b>${pairFinal}</b>...`);

    try {
      // 1. Получаем позицию
      const positions = await this._bot.exchange.fetchOpenPositions();
      const pos = positions.find(p => {
        const sym = p.symbol.replace(':USDT','').replace(':USD','');
        return (sym === pairFinal || sym === pairNorm) &&
               Math.abs(parseFloat(p.contracts || p.size || 0)) > 0;
      });
      if (!pos) {
        await this.sendMessage('❌ Позиция ' + pairFinal + ' не найдена');
        return;
      }

      const entry  = parseFloat(pos.entryPrice);
      const isLong = pos.side === 'long';
      const symbol = pos.symbol.replace(':USDT','').replace(':USD','');

      // 2. ATR14 по 1h свечам
      const candles = await this._bot.exchange.fetchCandles(symbol, '1h', 20);
      if (!candles || candles.length < 5) {
        await this.sendMessage('❌ Не удалось получить свечи для ' + symbol);
        return;
      }
      ...
      // 3. Герчик: SL = entry ± ATR, TP1/2/3 = entry ± 1/2/3 × ATR
      const dec = entry < 1 ? 4 : entry < 10 ? 3 : 2;
      const round = v => parseFloat(v.toFixed(dec));

      const sl  = round(isLong ? entry - atr       : entry + atr);
      const tp1 = round(isLong ? entry + atr       : entry - atr);
      const tp2 = round(isLong ? entry + atr * 2   : entry - atr * 2);
      const tp3 = round(isLong ? entry + atr * 3   : entry - atr * 3);

      // Проверка: SL на правильной стороне
      if (isLong && sl >= entry) {
        await this.sendMessage('❌ Ошибка расчёта SL: SL=' + sl + ' >= entry=' + entry);
        return;
      }
      if (!isLong && sl <= entry) {
        await this.sendMessage('❌ Ошибка расчёта SL: SL=' + sl + ' <= entry=' + entry);
      ...
      // 4. Ставим SL + TP3 на бирже
      const result = await this._bot.exchange.setTradingStop(symbol, {
      ...
      if (result && Number(result.retCode) !== 0) {
        await this.sendMessage('❌ Ошибка биржи: ' + result.retMsg);
        return;
      }

      // 5. Обновляем внутреннее состояние бота (TP1/TP2 для частичных закрытий)
      const botPos = this._bot.positions.get(symbol);
      if (botPos) {
        botPos.stopLoss    = sl;
        botPos.takeProfit  = tp3;
        botPos.tp1         = tp1;
        botPos.tp2         = tp2;
        botPos.tp3         = tp3;
        botPos._originalSL = sl;
        botPos._tp1Hit     = false;
        botPos._tp2Hit     = false;
        botPos._tp3Hit     = false;
      }

      const sideRu = isLong ? 'LONG' : 'SHORT';
      const hasBotPos = !!botPos;

      await this.sendMessage(
        '✅ <b>SL/TP установлены — ' + symbol + ' ' + sideRu + '</b>\n\n' +
        'Вход: <code>' + entry + '</code>\n' +
        'ATR14(1h): <code>' + atr.toFixed(5) + '</code>\n\n' +
        '🛑 SL:  <code>' + sl + '</code>\n' +
        '🎯 TP1: <code>' + tp1 + '</code> (1R — 30% бот)\n' +
        '🎯 TP2: <code>' + tp2 + '</code> (2R — 40% бот)\n' +
        '🎯 TP3: <code>' + tp3 + '</code> (3R — 30% биржа)\n\n' +
        'R:R 1:3.0\n' +
        (hasBotPos
          ? '✅ Частичные закрытия TP1/TP2 активны'
          : '⚠️ Позиция не в памяти бота — TP1/TP2 неактивны (только TP3 на бирже)')
      );

    } catch (e) {
      await this.sendMessage('❌ Ошибка: ' + e.message);
    }
  }

  async sendVolatilityReport(report, activePairs, positions) {
    if (!this._chatId) return;
    if (!report || !report.top10 || !report.top10.length) return;

    const ts = new Date(report.timestamp);
    const timeStr = ts.toUTCString().replace('GMT', 'UTC');

    let msg = '📊 <b>Скан волатильности завершён</b>\n';
    msg += '<i>' + timeStr + '</i>\n\n';
    msg += '<b>Топ-10 пар по волатильности:</b>\n\n';

    const buttons = [];

    for (let i = 0; i < report.top10.length; i++) {
      const r = report.top10[i];
      const inActive = activePairs.includes(r.pair);
      const hasPos = positions && positions.has(r.pair);
      const icon = hasPos ? '🔒' : (inActive ? '✅' : '·');
      const levelMark = r.hasNearLevel ? ' 🎯' : '';

      msg += (i + 1) + '. ' + icon + ' <b>' + r.pair + '</b>' + levelMark + '\n';
      msg += '   ATR: <code>' + r.atrPct.toFixed(2) + '%</code>  ';
      msg += 'Диапазон: <code>' + r.rangePct.toFixed(2) + '%</code>  ';
      msg += 'Объём: <code>' + r.volRatio.toFixed(2) + 'x</code>  ';
      msg += 'Скор: <code>' + r.score.toFixed(2) + '</code>\n\n';

      if (hasPos) {
        buttons.push([{ text: '🔒 ' + r.pair + ' — позиция', callback_data: 'noop' }]);
      } else {
        const label = inActive
          ? '✅ ' + r.pair + ' — выключить'
          : '· ' + r.pair + ' — включить';
        buttons.push([{ text: label, callback_data: 'mgr_toggle_' + r.pair.replace('/', '_') }]);
      }
    }

    buttons.push([{ text: '⚙️ Все пары (50)', callback_data: 'manage_pairs' }]);
    buttons.push([{ text: '◀️ Меню', callback_data: 'menu' }]);
