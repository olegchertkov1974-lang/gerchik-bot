...
// Usage: node generate-weekly-report.js [end_date]
// Generates weekly report for the 7 days ending on end_date (inclusive)
const dateArg   = process.argv[2];
const endDate   = dateArg || new Date().toISOString().slice(0, 10);
...
const trades = db.prepare(
  `SELECT * FROM trades WHERE date(closed_at) >= ? AND date(closed_at) <= ? ORDER BY closed_at ASC`
...
// PnL by day for weekly summary
function pnlByDay(trades, startDate, endDate) {
  const map = {};
  ...
  for (const t of trades) {
    ...
    if (map[day] !== undefined) map[day] += t.pnl || 0;
  }
  return map;
}
...
const dailyPnl   = pnlByDay(trades, startDate, endDate);
...
L.push('## 🏆 8. Лучшая и худшая сделка недели');
if (bestTrade) {
...
L.push('| Сплит TP1/TP2/TP3 | PnL недели | vs текущего |');
L.push('|-------------------|------------|-------------|');
...
L.push('---');
L.push(`_gerchik-bot · неделя ${weekLabel} · auto-generated_`);

const report  = L.join('\n');
const outName = `week-${endDate}.md`;
